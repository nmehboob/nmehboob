# Hi there, I'm Mahboob 👋
**Electrical Engineer | Embedded Systems | IoT | Control Engineering | BI & Data Analytics**

I'm an electrical engineer with hands-on experience in embedded systems and IoT, currently pursuing my M.Sc. in Control, Microsystems and Microelectronics at Universität Bremen. I work on projects where code meets hardware from sensor-based monitoring systems to power grid analysis and enjoy bridging firmware, signal processing, and software development to build systems that actually work end-to-end.

📍 **Location:** Bremen, Germany  
📧 **Email:** nnmehboob1233@gmail.com  
💼 **GitHub:** [@nmehboob](https://github.com/nmehboob)  
🔗 **LinkedIn:** [mahboobnawaz1](https://www.linkedin.com/in/mahboobnawaz1/)

---

## 🚀 Featured Projects

### 📡 IoT Vibration Monitoring System
**Real-time machine health monitoring | IGNITE Technology Fund Grantee 🏆**

Built an ESP32-based vibration monitoring system that captures real-time accelerometer data from rotating machinery and streams it to the cloud for anomaly detection. Selected for the IGNITE Technology Fund (Pakistan's national tech grant program), validating both the technical approach and commercial potential.

- Real-time vibration data acquisition using ESP32 and accelerometer sensors
- Cloud-based dashboard for remote monitoring and alerts
- FFT-based spectral analysis for early fault detection
- Designed for industrial rotating equipment (motors, pumps, fans)

**Tech:** ESP32, C/C++, FFT, IoT Protocols, Sensor Integration, Cloud Dashboard

---

### ⚡ Power Systems Protection & Optimization
**Grid analysis and protection coursework, M.Sc.**

Power systems coursework covering protection schemes, economic dispatch, and grid stability analysis. Implemented distance protection logic, set up and solved optimal power flow problems, and ran transient and small-signal stability studies on standard test networks.

- Distance protection coordination for transmission lines
- Economic Dispatch and Optimal Power Flow (OPF) formulations
- Transient and small-signal stability analysis
- Network modeling and fault simulation in DIgSILENT PowerFactory

**Tech:** DIgSILENT PowerFactory, MATLAB, Python, Optimization

---

### 📊 Trebel Corp Business Intelligence Dashboard
**Sales, marketing, and customer analytics**

Built BI dashboards and reporting workflows at Trebel Corp covering sales performance, marketing campaign tracking, and customer analytics. Translated raw operational data into visual reports that supported day-to-day decisions across sales and marketing teams.

- Sales and revenue tracking with channel-level breakdowns
- Marketing campaign performance and ROI analysis
- Customer and audience segmentation reports
- Automated Excel pipelines feeding into Power BI dashboards

**Tech:** Power BI, Power Query, DAX, Advanced Excel, Data Modeling

---

### 🎛️ Control Systems: Rotary Inverted Pendulum
**Advanced controller design, M.Sc. coursework**

Coursework project designing and comparing controllers for a rotary inverted pendulum, a classic nonlinear unstable system. Implemented LQR, LQG, and H∞ controllers in MATLAB/Simulink and compared trade-offs between optimality, robustness, and noise rejection.

- State-space modeling and linearization
- LQR optimal control design
- LQG with Kalman filtering for state estimation under noise
- H∞ robust control for disturbance rejection
- Comparative performance analysis across controllers

**Tech:** MATLAB, Simulink, Control Theory, State-Space Methods

---

## 💻 Technical Skills

**Embedded & Hardware**
ESP32 · Arduino · STM32 · Firmware Development · Sensor Integration (accelerometers, temperature, current sensors) · Real-Time Data Acquisition · Oscilloscope & Multimeter

**Electrical Systems**
Power Systems Analysis · Protection Coordination · Motor Drives · Inverters · Power Electronics · Analog & Digital Circuits

**Control & Signal Processing**
LQR · LQG · H∞ Control · Kalman Filtering · FFT · Spectral Analysis · Vibration Analysis · Anomaly Detection · State-Space Methods

**Programming Languages**
Python · C/C++ · MATLAB · TypeScript · JavaScript

**Software & Web Development**
Angular · Socket.IO · REST APIs · Real-Time Systems · Optimistic UI Patterns

**AI / ML**
Groq · Llama 3.3 · Prompt Engineering · LangChain · LLM Application Development

**Engineering Tools**
DIgSILENT PowerFactory · Simulink · MATLAB · Git · Power BI · Advanced Excel · SAP R/3

---

## 🎯 What I Bring

I work at the intersection of hardware and software. My background spans embedded systems, control theory, and modern web development, which means I'm comfortable moving between firmware, signal processing, and application code on the same project.

- **Hardware-Software Bridge** – Comfortable across the stack, from ESP32 firmware to Angular frontends
- **Validated Track Record** – IGNITE-funded IoT project demonstrates ability to build real, fundable hardware
- **Control & Signal Processing Foundation** – Solid mathematical background in modeling, FFT-based analysis, and controller design
- **Day-to-Day Software Experience** – Daily Angular work on a real-time, multi-user application alongside academic projects

---

## 📫 Let's Connect

I'm interested in roles combining embedded systems, IoT, and control engineering, especially in industrial automation, energy systems, or any application where reliable hardware meets intelligent software.

📧 **Email:** nnmehboob1233@gmail.com  
💼 **GitHub:** [@nmehboob](https://github.com/nmehboob)  
🔗 **LinkedIn:** [mahboobnawaz1](https://www.linkedin.com/in/mahboobnawaz1/)

⭐️ From @nmehboob
